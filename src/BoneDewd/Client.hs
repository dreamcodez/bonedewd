...
newClientManagerState :: TChan (Client,RxPacket) -> ClientManagerState
...
initLoginClient (ClientManagerState clients next_cid worldChan) (peer,host,_) = do
    ...
    inbox <- atomically newTChan -- initialize client inbox
    let newClient = Client next_cid host (atomically . writeTChan inbox) (hClose peer)
...
clientReader :: ParseState -> Client -> Handle -> TChan (Client,RxPacket) -> IO ()
